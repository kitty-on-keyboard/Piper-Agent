#!/bin/zsh
# Build the neutral scoreboard once per SpecVerifier entrant and print one row each.
#
#   ./score.sh <dir-of-entrant-trees>
#
# Each subdirectory is one entrant, laid out as Brief C specified: include/ and src/.
...
for d in "$root"/*(/N) "$here"/falsifiers/*(/N); do
  n="${d:t}"
  inc=()
  # Brief C specified include/spec_verifier.hpp. One entrant shipped
  # include/kv/spec_verifier.hpp instead; both are put on the search path rather than
  ...
  [[ -f "$d/spec_verifier.hpp" ]] && inc+=("-I$d")
  srcs=("$d"/src/*.cpp(N) "$d"/spec_verifier.cpp(N))
  out="${TMPDIR:-/tmp}/sv_score_$n"
