...
class GrammarSink final : public model::TokenSink {
  public:
    ...
    //
    // The think budget is NOT enforced here. It used to be, by flipping the grammar's
    // phase from under the model -- see model::ThinkCapMask, which owns it now and forces
    // a real `</think>` through the mask instead. A sink observes tokens; it cannot append
    // one, so it was never the right place to end a phase.
    GrammarSink(model::TurnGrammar& g, TokenStreamer* streamer)
        : g_(g), streamer_(streamer) {}
