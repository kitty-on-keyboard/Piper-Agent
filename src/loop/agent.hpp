...
struct AgentConfig {
    ...
    // setenv. Product stays off until Mac live A/B KEEP.
    bool pulse = false;

    // Binary residual: force_tool iff p_force >= this floor, else nudge.
    // Seed defaults: 27B dense 0.55; A3B/MoE 0.50 (see pulse_default_p_min).
    // `LMP_PULSE_P_MIN` (float) overrides when set.
    float pulse_p_min = kPulseDefaultPMin;
