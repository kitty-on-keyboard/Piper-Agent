...
static constexpr const char* kWorkerHelpText =
    ...
    "pass --orch-webhook or stay attached. Two legal ways to own the horizon: stay attached "
    "(parent waits on files/exit, no webhook) or detach (screen, nohup, background, "
    "requiring a wake URL via --orch-webhook, task.json orch_webhook, or LMP_ORCH_WEBHOOK). "
    "A run that writes result.json POSTs done if completed, stalled if stopped/failed. "
    "Process exit with no result.json POSTs died. Silent detached workers are refused.\n";
} // namespace
