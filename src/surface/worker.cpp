...
std::optional<TaskPacket> load_packet(const std::string& path_in, std::string& error) {
    ...
    if (j.contains("max_iterations")) {
        if (j["max_iterations"].is_boolean() || !j["max_iterations"].is_number()) {
...
std::string build_start_message(const TaskPacket& packet, const std::string& request_id) {
    nlohmann::json settings = {
        ...
        {"mode", packet.mode},
        {"max_iterations", resolve_max_iterations(packet)},
...
bool is_parent_harness_command(std::string_view cmd) {
    return cmd == "packet" || cmd == "dispatch" || cmd == "answer" ||
           cmd == "progress" || cmd == "review" || cmd == "status" ||
           cmd == "await" || cmd == "mcp-list" || cmd == "wake-url" ||
           cmd == "init";
}

bool flag_takes_value(std::string_view arg) {
    return arg == "--task" || arg == "--orch-webhook" || arg == "--socket" ||
           arg == "--idle-timeout" || arg == "--dir";
}

std::optional<std::filesystem::path> parent_harness_script() {
    ...
    std::error_code ec;
    std::filesystem::path dir = std::filesystem::weakly_canonical(buf, ec);
    if (ec) dir = std::filesystem::path(buf);
    dir = dir.parent_path();
    for (int i = 0; i < 12; ++i) {
        std::filesystem::path candidate = dir / "scripts" / "piper_worker.py";
...
std::optional<int> forward_parent_harness(int argc, char** argv) {
    if (argc <= 1 || argv == nullptr) return std::nullopt;
    bool forward = false;
    for (int i = 1; i < argc; ++i) {
        std::string_view arg = argv[i] ? argv[i] : "";
        if (arg == "--worker") return std::nullopt;
        ...
        if (arg == "worker") {
            if (i + 1 < argc && argv[i + 1] &&
                std::string_view(argv[i + 1]) == "init") {
                forward = true;
            }
            break;
        }
        forward = is_parent_harness_command(arg);
        break;
    }
    if (!forward) return std::nullopt;

    std::optional<std::filesystem::path> script = parent_harness_script();
    if (!script) {
        std::fprintf(stderr, "piper: parent harness not found (scripts/piper_worker.py)\n");
        return kExitError;
    }
    std::string script_s = script->string();
    std::vector<char*> args;
    args.reserve(static_cast<std::size_t>(argc) + 2);
    args.push_back(const_cast<char*>("python3"));
    args.push_back(script_s.data());
    for (int i = 1; i < argc; ++i) {
        if (argv[i] && std::string_view(argv[i]) == "--worker") continue;
        args.push_back(argv[i]);
    }
    args.push_back(nullptr);
    ::execvp("python3", args.data());
    std::fprintf(stderr, "piper: failed to exec parent harness: %s\n", std::strerror(errno));
    return kExitError;
}
...
int init_project(const std::string& target_dir_in) {
    std::optional<std::filesystem::path> script = parent_harness_script();
    if (!script) {
        std::fprintf(stderr, "piper init: parent harness not found (scripts/piper_worker.py)\n");
        return kExitError;
    }
    std::string dir = target_dir_in.empty() ? "." : target_dir_in;
    std::string script_s = script->string();
    pid_t pid = ::fork();
    if (pid < 0) {
        std::fprintf(stderr, "piper init: fork failed: %s\n", std::strerror(errno));
        return kExitError;
    }
    if (pid == 0) {
        ::execlp("python3", "python3", script_s.c_str(), "init", dir.c_str(),
                 static_cast<char*>(nullptr));
        std::fprintf(stderr, "piper init: failed to exec parent harness: %s\n",
                     std::strerror(errno));
        ::_exit(127);
    }
    int status = 0;
    if (::waitpid(pid, &status, 0) < 0) {
        std::fprintf(stderr, "piper init: wait failed: %s\n", std::strerror(errno));
        return kExitError;
    }
