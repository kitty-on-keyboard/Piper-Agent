...
## 2026-06-05 - Centralizing zero-allocation integer formatting for tool outputs
**Learning:** `std::to_string` triggers costly temporary heap allocations and locale overhead during string concatenation, which is especially noticeable in high-traffic tool loops like edit diagnostics formatting, patch line numbering, and log triaging. While some files duplicated an `append_num` helper leveraging `std::to_chars` into local buffers, many `std::to_string` usages persisted. Consolidating this utility to avoid redefinition errors (e.g. sharing `lmp::tools::append_num` via `src/tools/text_view.hpp`) is essential to preventing compilation issues when including multiple headers, while cleanly replacing `std::to_string` across files.
**Action:** Replace `std::to_string` string concatenations with `lmp::tools::append_num` in output formatters to guarantee zero-allocation string accumulation. Refactor awkward inline lambdas (IIFEs) intended to scope `append_num` by directly initializing and appending to the target string variable.
