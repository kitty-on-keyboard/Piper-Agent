...
## 2026-05-30 - Single-Pass Bitmask Pre-Computation for Anchor Match Scoring
**Learning:** In `nearest_regions` tie-breaking, checking first-line anchor token alignment previously re-tokenized line tokens inside the sliding window evaluation loop using `tokenize_cb` on every candidate window evaluation. Pre-computing anchor presence bitmasks (`anchor_mask`) and hits during initial single-pass line tokenization allows $O(1)$ bitwise popcount queries (`std::popcount(line_info[j].anchor_mask)`), speeding up `nearest_regions` region scoring by ~18% (from ~10.9s to ~9.0s across 1000 iterations over 5000 lines).
**Action:** Pre-compute auxiliary tie-breaking bitmasks during initial single-pass line tokenization instead of re-tokenizing lines during per-window evaluation loops.
