...
## 2026-04-18 - Fast Hash Bitmask Duplicate Key Filtering and Key-Length Early Exit in parsephony
**Learning:** In `parsephony`, `Parser::parse` artificially marked any object with $\ge 32$ keys as having duplicate keys (`kDuplicateKeys`) to avoid $O(N^2)$ linear comparisons during parsing. This forced `Value::operator[](key)` on large objects to perform full $O(N)$ scans without early exit. Maintaining a 64-bit hash filter (`uint64_t seen_key_hashes`) in `Parser::Frame` during parse reduces string comparison checks by ~98.4%, ensuring `kDuplicateKeys` is set only when duplicate keys actually exist. In addition, checking key length (`kn.len == key.size()`) before string equality evaluation in `Value::operator[]` skips non-matching length keys instantly.
**Action:** Use fast bitmask bloom filters during object parsing for $O(1)$ duplicate key filtering, and check key length before string content matching during JSON property lookups.
