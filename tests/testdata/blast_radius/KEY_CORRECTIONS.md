...
`tests/corpus/test_corpus_loads.cpp` asserts that exactly the corrections listed here are
...
```bash
cmake --build build --target blast_radius_score_shipped && ./build/tests/testdata/blast_radius_score_shipped
```
