...
// testdata root and neither can shadow the other.
...
std::vector<BrCase> load_br(const std::string& file) {
    ...
    std::string err;
    const bool ok =
        blast_radius_corpus::load_corpus(std::string(LMP_TESTDATA_BR_DIR) + "/" + file,
                                         cases, err);
    if (!ok) {
...
TEST(log_triage_corpus_loads_with_every_log_present) {
    const std::vector<LtCase> cases = log_triage_corpus::load(LMP_TESTDATA_LT_DIR, "corpus.jsonl");
...
TEST(log_triage_holdout_loads_and_is_disjoint) {
    const std::vector<LtCase> corpus = log_triage_corpus::load(LMP_TESTDATA_LT_DIR, "corpus.jsonl");
    const std::vector<LtCase> holdout = log_triage_corpus::load(LMP_TESTDATA_LT_DIR, "holdout.jsonl");
