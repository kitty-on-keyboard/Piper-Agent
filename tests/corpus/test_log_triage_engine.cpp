...
//
// WHY THIS FILE EXISTS. Until 2026-07-31 nothing tested the engine at all. The corpus
...
std::vector<LtCase> load(const std::string& manifest) {
    std::vector<LtCase> cases = log_triage_corpus::load(LMP_TESTDATA_LT_DIR, manifest);
    if (cases.empty()) {
